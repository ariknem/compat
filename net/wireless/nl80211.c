...
static const struct nla_policy nl80211_policy[NL80211_ATTR_MAX+1] = {
	...
	[NL80211_ATTR_TDLS_TYPE] = { .type = NLA_U8 },
};
...
static int nl80211_new_station(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (dev->ieee80211_ptr->iftype != NL80211_IFTYPE_AP &&
	    ...
	    dev->ieee80211_ptr->iftype != NL80211_IFTYPE_STATION) /* TDLS STATIONS CAN ADD STATIONS */
	    return -EINVAL;
	err = get_vlan(info, rdev, &params.vlan);
	if (err)
		goto out;
...
/* TDLSTODO: add some check for the presense of every attr */
static int nl80211_tdls_mgmt(struct sk_buff *skb, struct genl_info *info)
{
	...
	action_code = nla_get_u8(info->attrs[NL80211_ATTR_TDLS_TYPE]);
	if(info->attrs[NL80211_ATTR_MAC])
		peer = nla_data(info->attrs[NL80211_ATTR_MAC]);
	status_code = nla_get_u16(info->attrs[NL80211_ATTR_STATUS_CODE]);
	dialog_token = nla_get_u8(info->attrs[NL80211_ATTR_MGMT_SUBTYPE]);

	err = rdev->ops->tdls_mgmt(&rdev->wiphy, dev, peer, action_code, dialog_token, status_code, 
...
static int nl80211_tdls_oper(struct sk_buff *skb, struct genl_info *info)
{
	...
	tdls_operation = nla_get_u8(info->attrs[NL80211_ATTR_TDLS_TYPE]);

	if (info->attrs[NL80211_ATTR_MAC])
		peer = nla_data(info->attrs[NL80211_ATTR_MAC]);

	err = rdev->ops->tdls_oper(&rdev->wiphy, dev, peer, tdls_operation);
	if (!err) {
		...
		NLA_PUT_U16(msg, NL80211_ATTR_STATUS_CODE, err);
		if ((err = genlmsg_end(msg, hdr)) < 0) {
			nlmsg_free(msg);
			return err;
		} else
			return genlmsg_reply(msg, info);
